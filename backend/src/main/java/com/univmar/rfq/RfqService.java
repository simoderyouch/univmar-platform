...
import com.univmar.customer.domain.CustomerProjectRepository;
...
public class RfqService {
    ...
    private final QuoteRequestItemRepository items;
    private final UserRepository users;
    private final StoneVariantRepository variants;
    private final CustomerProjectRepository projects;

    public RfqService(QuoteRequestRepository r, QuoteRequestItemRepository i, UserRepository u, StoneVariantRepository v, CustomerProjectRepository p) {
        requests = r;
        items = i;
        users = u;
        variants = v;
    ...
    @Transactional
    public RfqDtos.Response submit(Long customerId, Long id) {
        QuoteRequest request = owned(customerId, id);
        if (items.findByRequestId(id).isEmpty())
            throw ApiException.conflict("EMPTY_RFQ", "RFQ must contain at least one item");
        try {
            request.submit();
    ...
    @Transactional
    public RfqDtos.Response cancel(Long customerId, Long id) {
        QuoteRequest request = owned(customerId, id);
        try {
            request.cancel();
        } catch (IllegalStateException e) {
    ...
    public List<RfqDtos.Detail> mine(Long customerId) {
        return requests.findByCustomerId(customerId).stream().map(this::detail).toList();
    }

    @Transactional(readOnly = true)
    public RfqDtos.Detail mineDetail(Long customerId, Long id) {
        return detail(owned(customerId, id));
    }

    @Transactional(readOnly = true)
    public Page<RfqDtos.Detail> sales(Pageable pageable) {
        return requests.findAll(pageable).map(this::detail);
    }

    @Transactional(readOnly = true)
    public RfqDtos.Detail salesDetail(Long id) {
        return detail(find(id));
    }

    @Transactional
    public RfqDtos.Detail assignAndReview(Long id, Long salesUserId) {
        QuoteRequest request = find(id);
        var sales = users.findById(salesUserId).orElseThrow(() -> ApiException.notFound("Sales user"));
        if (sales.getRole() != Role.SALES && sales.getRole() != Role.ADMIN)
            throw ApiException.conflict("INVALID_ASSIGNEE", "Assignee must be a Sales user or Admin");
        try {
            request.review(salesUserId);
        } catch (IllegalStateException e) {
            throw ApiException.conflict("INVALID_STATE_TRANSITION", e.getMessage());
        }
    ...
    @Transactional
    public RfqDtos.Detail close(Long id) {
        QuoteRequest request = find(id);
        try {
            request.close();
        } catch (IllegalStateException e) {
    ...
    private QuoteRequest owned(Long customerId, Long id) {
