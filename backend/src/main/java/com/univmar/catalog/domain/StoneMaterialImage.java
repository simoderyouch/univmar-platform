...
public class StoneMaterialImage extends BaseEntity {
    @ManyToOne(optional = false) @JoinColumn(name = "material_id", nullable = false)
    private StoneMaterial material;
    @Column(nullable = false, length = 1000) private String imageUrl;
    @Column(unique = true, length = 255) private String storageKey;
    ...
    public void updatePresentation(String altText, int displayOrder, boolean primaryImage) { this.altText = altText; this.displayOrder = displayOrder; this.primaryImage = primaryImage; }
}
