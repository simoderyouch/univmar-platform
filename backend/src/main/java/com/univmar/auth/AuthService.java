...
public class AuthService {
    private final UserRepository users; private final CustomerProfileRepository profiles; private final RefreshTokenRepository refreshTokens;
    private final PasswordEncoder passwords; private final JwtService jwt; private final SecureRandom random = new SecureRandom();
    public AuthService(UserRepository users, CustomerProfileRepository profiles, RefreshTokenRepository refreshTokens, PasswordEncoder passwords, JwtService jwt) {
        this.users = users; this.profiles = profiles; this.refreshTokens = refreshTokens; this.passwords = passwords; this.jwt = jwt;
    }
    @Transactional
    public AuthDtos.TokenResponse register(AuthDtos.RegisterRequest request) {
        String email = request.email().trim().toLowerCase();
        if (users.existsByEmailIgnoreCase(email)) throw ApiException.conflict(ErrorCode.EMAIL_ALREADY_EXISTS, "An account already uses this email");
    ...
    public AuthDtos.TokenResponse refresh(String token) {
        ...
        if (!refresh.isUsable() || refresh.getUser().getStatus() != AccountStatus.ACTIVE) throw new ApiException(HttpStatus.UNAUTHORIZED, ErrorCode.INVALID_REFRESH_TOKEN, "Refresh token is expired or revoked");
        refresh.revoke(); return issue(refresh.getUser());
    }
    @Transactional
    public void logout(String token) { refreshTokens.findByToken(token).ifPresent(RefreshToken::revoke); }
