...
function VariantManager({ variants, create, archive }: { variants: Variant[]; create(input: { finish: string; thicknessMm: number; grade: string; indicativePrice: number | null }): void; archive(id: number): void }) {
  ...
  return <div className="mt-6 border-t border-[#e4ddd2] pt-5"><b className="text-sm text-black">Commercial variants</b><div className="mt-3 grid gap-2 sm:grid-cols-4"><Input placeholder="Finish" value={finish} onChange={(event) => setFinish(event.target.value)} /><Input type="number" min="0.01" placeholder="Thickness mm" value={thicknessMm} onChange={(event) => setThickness(event.target.value)} /><Input placeholder="Grade" value={grade} onChange={(event) => setGrade(event.target.value)} /><Input type="number" min="0" placeholder="Price" value={price} onChange={(event) => setPrice(event.target.value)} /></div><Button size="sm" className="mt-3" onClick={() => { if (!finish.trim() || !Number(thicknessMm)) return; create({ finish: finish.trim(), thicknessMm: Number(thicknessMm), grade, indicativePrice: price ? Number(price) : null }); setFinish(""); setGrade(""); setPrice(""); }}>Add variant</Button><div className="mt-4 divide-y divide-[#ede8df] rounded-md border border-[#e4ddd2]">{variants.length ? variants.map((variant) => <div key={variant.id} className="flex items-center justify-between gap-3 px-3 py-2 text-xs"><span className="text-[#6b6560]"><b className="text-black">{variant.finish}</b> · {variant.thicknessMm} mm · {variant.grade || "Standard"} · {variant.indicativePrice ?? "—"}</span><Button size="sm" variant="ghost" onClick={() => archive(variant.id)}>Hide</Button></div>) : <p className="p-3 text-xs text-[#7a736c]">No variants yet.</p>}</div></div>;
}
